...
PLATFORM_CHOICE, STREAM_NAME, M3U8_LINK, STREAM_KEY = range(4)
processes = {}
...
async def start_prepare(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [InlineKeyboardButton("📡 Live IG", callback_data="ig")],
        [InlineKeyboardButton("📺 Live FB", callback_data="fb")]
    ]
    markup = InlineKeyboardMarkup(buttons)
    await update.message.reply_text("اختر المنصة للبث:", reply_markup=markup)
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ تم بدء البث على {'Instagram' if platform == 'ig' else 'Facebook'}!\n📛 الاسم: {name}")

    if not is_subscribed(update.effective_user.id):
        def stop_and_notify():
            stop_stream_process(tag)
            asyncio.run_coroutine_threadsafe(
                update.message.reply_text("⏰ انتهى وقت البث المجاني (30 دقيقة). يرجى الاشتراك لمواصلة البث."),
                context.application.loop
            )
            asyncio.run_coroutine_threadsafe(
                context.bot.send_message(chat_id=ADMIN_CHAT_ID, text=f"📢 المستخدم @{update.effective_user.username or update.effective_user.id} انتهى بثه المجاني."),
                context.application.loop
            )
        threading.Timer(1800, stop_and_notify).start()
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            PLATFORM_CHOICE: [CallbackQueryHandler(choose_platform)],
