...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = ReplyKeyboardMarkup(
        [["🎬 تجهيز البث FB", "🎬 تجهيز البث IG"], ["⏹ إيقاف البث", "🔁 إعادة تشغيل البث"]],
...
async def start_prepare_fb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    allowed, msg = can_stream(update.effective_user.id)
    if not allowed:
        await update.message.reply_text(msg)
        return ConversationHandler.END
    await update.message.reply_text("🎥 أرسل اسم البث (FB):")
    context.user_data["stream_type"] = "FB"
    return STREAM_NAME
...
async def start_prepare_ig(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not allowed:
        await update.message.reply_text(msg)
        return ConversationHandler.END
    await update.message.reply_text("🎥 أرسل اسم البث (IG):")
    context.user_data["stream_type"] = "IG"
    return STREAM_NAME
...
async def get_m3u8(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("stream_type") == "FB":
        await update.message.reply_text("🔑 أرسل مفتاح البث FB (يبدأ بـ FB-):")
        return FB_KEY
    else:  # IG
        await update.message.reply_text("🔑 أرسل مفتاح البث IG:")
        return IG_KEY
...
async def get_fb_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not key.startswith("FB-"):
        ...
        return ConversationHandler.END
    return await start_stream(update, context, key, "FB")

async def get_ig_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    key = update.message.text.strip()
    # لا تحقق مفتاح IG فقط استقبل المفتاح كما هو
    return await start_stream(update, context, key, "IG")

async def start_stream(update: Update, context: ContextTypes.DEFAULT_TYPE, key: str, stream_type: str):
    ...
    tag = f"{user_id}_{name}_{stream_type}"
    ...
    user["last_stream_info"] = {"m3u8": link, "key": key, "name": name, "type": stream_type}
    users[user_id] = user
    save_json(USERS_FILE, users)

    await update.message.reply_text(f"✅ تم بدء بث {stream_type}!\n📛 الاسم: {name}")
...
async def restart_stream(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    key = user["last_stream_info"]["key"]
    stream_type = user["last_stream_info"].get("type", "FB")
    if stream_type == "FB":
        return await start_stream(update, context, key, "FB")
    else:
        return await start_stream(update, context, key, "IG")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث FB":
        return await start_prepare_fb(update, context)
...
def main():
    ...
    conv_handler_fb = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🎬 تجهيز البث FB$"), start_prepare_fb)],
        states={
