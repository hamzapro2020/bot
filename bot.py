...
# إعدادات
TOKEN = os.getenv("TOKEN")  # ضع التوكن هنا مباشرة أو في متغير بيئي
...
ADMIN_CHAT_ID = -1001234567890  # شات المشرفين للإشعارات

os.makedirs("data", exist_ok=True)

# مراحل المحادثة
PLATFORM, M3U8_LINK, FB_KEY = range(3)
...
def can_stream(user_id):
    ...
    if not last_date or last_date.date() < now.date():
        usage = 0
    if usage >= 1:
        return False, "❌ وصلت الحد المجاني اليومي للبث. يرجى الاشتراك للبث أكثر."
    return True, ""
...
async def get_platform(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if platform not in ["البث Facebook", "البث Instagram"]:
        await update.message.reply_text("❌ يرجى اختيار أحد الخيارين.")
        return ConversationHandler.END
    
    context.user_data["platform"] = platform
    
    # توليد اسم بث عشوائي بدل طلب الاسم
    stream_name = generate_random_id(10)
    context.user_data["stream_name"] = stream_name

    await update.message.reply_text(f"تم توليد اسم البث تلقائياً: `{stream_name}`\n\n🔗 أرسل رابط M3U8:")
    return M3U8_LINK
...
async def get_fb_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    platform = context.user_data.get("platform", "البث Facebook")

    # بناء رابط البث حسب المنصة
    if platform == "البث Facebook":
...
async def restart_stream(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إعادة تشغيل البث مع نفس البيانات
...
# إدارة الاشتراكات (مثال، يحتاج تعديل حسب الحاجة)
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("📥 تحميل ملف IPTV"), download_and_send_iptv))
