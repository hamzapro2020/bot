...
ADMINS = [8145101051]  # استبدل هذا برقم الأدمن الخاص بك
...
async def get_api_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if response.status_code != 200 or not response.json().get("stream_url"):
...
async def add_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❗ أرسل ID المستخدم الذي تريد ترقيته.")
        return

    user_id = context.args[0]
    users = load_json(USERS_FILE)
    users[user_id] = users.get(user_id, {})
    users[user_id]["expires"] = (datetime.now() + timedelta(days=30)).isoformat()
    save_json(USERS_FILE, users)
    await update.message.reply_text(f"✅ تم ترقية المستخدم {user_id} لمدة 30 يومًا.")
...
async def remove_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❗ أرسل ID المستخدم الذي تريد إلغاء اشتراكه.")
        return

    user_id = context.args[0]
    users = load_json(USERS_FILE)
    if user_id in users and "expires" in users[user_id]:
        users[user_id].pop("expires")
        save_json(USERS_FILE, users)
        await update.message.reply_text(f"✅ تم إلغاء اشتراك المستخدم {user_id}.")
    else:
        await update.message.reply_text("❌ المستخدم غير موجود أو ليس مشتركًا.")

# ===== بدء البوت =====
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "مرحبًا! استخدم الأزرار لتجهيز أو إيقاف البث.",
